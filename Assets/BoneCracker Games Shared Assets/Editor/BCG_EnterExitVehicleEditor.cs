...
public class BCG_EnterExitVehicleEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        EditorGUILayout.HelpBox("Select proper camera for each vehicle. Selecting RCCP Camera for RCCP vehicles, selecting RTC Camera for RTC vehicles, etc...", MessageType.Info);
        EditorGUILayout.Space();

        if (!EditorUtility.IsPersistent(prop)) {

            if (!FindFirstObjectByType<BCG_EnterExitManager>()) {
