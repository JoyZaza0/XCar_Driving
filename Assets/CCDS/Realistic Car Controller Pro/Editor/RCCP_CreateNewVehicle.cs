...
public class RCCP_CreateNewVehicle {
    ...
    public static RCCP_CarController NewVehicle(GameObject vehicle) {
        ...
        if (!Selection.activeGameObject.TryGetComponent(out CCDS_Player playerScript))
            Selection.activeGameObject.AddComponent<CCDS_Player>();



        int answer2 = EditorUtility.DisplayDialogComplex("Is This Vehicle Will Be Player Car?", "Is this vehicle will be used for player car, or AI cop car behavior?", "Player Car", "AI Cop Car", "");

        if (!Selection.activeGameObject.TryGetComponent(out CCDS_AI_Cop copScript)) {

            if (answer2 == 1)
                Selection.activeGameObject.AddComponent<CCDS_AI_Cop>();
