...
public class RCCP_AI : RCCP_Component {
        ...
        switch (navigationMode) {
            ...
            case NavigationMode.FollowWaypoints:
                ...
                if (!reversingNow) {
                    ...
                    throttleInput = (distanceToNextWaypoint < (nextWaypointPassDistance * (CarController.absoluteSpeed) / 30f)) ? (Mathf.Clamp01(currentWaypoint.targetSpeed - CarController.absoluteSpeed)) : 1f;

                    if (limitSpeed)
                        throttleInput *= Mathf.Clamp01(Mathf.Lerp(10f, 0f, (CarController.speed) / maximumSpeed));

                    brakeInput = (distanceToNextWaypoint < (nextWaypointPassDistance * (CarController.absoluteSpeed) / 30f)) ? (Mathf.Clamp01(CarController.absoluteSpeed - currentWaypoint.targetSpeed)) : 0f;
                    handbrakeInput = 0f;
            ...
            case NavigationMode.ChaseTarget:
                ...
                if (!reversingNow) {
                    ...
                    if (limitSpeed)
                        throttleInput *= Mathf.Clamp01(Mathf.Lerp(10f, 0f, (CarController.absoluteSpeed) / maximumSpeed));
            ...
            case NavigationMode.FollowTarget:
                ...
                if (!reversingNow) {
                    ...
                    throttleInput = distanceToTarget < (stopFollowDistance * Mathf.Lerp(1f, 5f, CarController.absoluteSpeed / 50f)) ? Mathf.Lerp(-5f, 1f, distanceToTarget / (stopFollowDistance / 1f)) : 1f;

                    if (limitSpeed)
                        throttleInput *= Mathf.Clamp01(Mathf.Lerp(10f, 0f, (CarController.absoluteSpeed) / maximumSpeed));

                    brakeInput = distanceToTarget < (stopFollowDistance * Mathf.Lerp(1f, 5f, CarController.absoluteSpeed) / 50f) ? Mathf.Lerp(5f, 0f, distanceToTarget / (stopFollowDistance / 1f)) : 0f;
                    handbrakeInput = 0f;
    ...
    private void CheckReset() {
        ...
        if (CarController.absoluteSpeed <= 5)
        ...
        if (resetTime >= 4 || CarController.absoluteSpeed >= 25) {
