...
public class RCCP_WheelCollider : RCCP_Component {
    ...
    private void Frictions() {
        ...
        if (isGrounded) {
            ...
            wheelSlipAmountForward = Mathf.Lerp(wheelSlipAmountForward, wheelHit.forwardSlip, Time.fixedDeltaTime * 15f);
            wheelSlipAmountSideways = Mathf.Lerp(wheelSlipAmountSideways, wheelHit.sidewaysSlip, Time.fixedDeltaTime * 15f);
            totalSlip = Mathf.Lerp(totalSlip, (Mathf.Abs(wheelHit.forwardSlip) + Mathf.Abs(wheelHit.sidewaysSlip)), Time.fixedDeltaTime * 15f);

        } else {

            wheelSlipAmountForward = Mathf.Lerp(wheelSlipAmountForward, 0f, Time.fixedDeltaTime * 15f);
            wheelSlipAmountSideways = Mathf.Lerp(wheelSlipAmountSideways, 0f, Time.fixedDeltaTime * 15f);
            totalSlip = Mathf.Lerp(totalSlip, 0f, Time.fixedDeltaTime * 15f);
        ...
        if (skidAudioSource) {
            ...
            if (totalSlip > RCCPGroundMaterials.frictions[groundIndex].slip) {
                ...
                if (CarController.Rigid.linearVelocity.magnitude > .1f) {
                    ...
                    skidAudioSource.volume = Mathf.Lerp(skidAudioSource.volume, Mathf.Lerp(0f, skidVolume, totalSlip - RCCPGroundMaterials.frictions[groundIndex].slip), Time.fixedDeltaTime * 5f);
                    skidAudioSource.pitch = Mathf.Lerp(skidAudioSource.pitch, Mathf.Lerp(.7f, 1f, totalSlip - RCCPGroundMaterials.frictions[groundIndex].slip), Time.fixedDeltaTime * 5f);
